...
static int jl_prune_tcache(jl_tuplemap_entry_t *ml, void *closure)
{
...
static void jl_serialize_value_(ios_t *s, jl_value_t *v)
{
    ...
    if (jl_is_svec(v)) {
        ...
        union jl_tuplemap_t *tf = &li->tfunc;
        if (tf->unknown && tf->unknown != jl_nothing) {
            if (li->module == jl_core_module) {
               tf->unknown = jl_nothing;
                ...
                jl_tuplemap_visitor(*tf, jl_prune_tcache, NULL);
            }
        }
...
static int jl_serialize_methcache_from_mod(jl_tuplemap_entry_t *ml, void *closure)
{
...
static void jl_serialize_methtable_from_mod(ios_t *s, jl_methtable_t *mt, int8_t iskw)
{
    ...
    jl_tuplemap_visitor(mt->defs, jl_serialize_methcache_from_mod, &env);
}
...
static int remove_specializations_from_replaced_modules_visitor(jl_tuplemap_entry_t *l, void *closure)
{
...
static void remove_methods_from_replaced_modules_from_list(jl_tuplemap_entry_t **pl)
{
...
static void remove_methods_from_replaced_modules_from_cache(union jl_tuplemap_t *tc);
static void remove_methods_from_replaced_modules_from_array(jl_array_t *a)
{
    ...
    for(i=0; i < l; i++) {
        if (data[i] != NULL)
            remove_methods_from_replaced_modules_from_cache((union jl_tuplemap_t*)&data[i]);
    }
}
...
static void remove_methods_from_replaced_modules_from_cache(union jl_tuplemap_t *tc)
{
    ...
    if (jl_typeof(tc->unknown) == (jl_value_t*)jl_tuplemap_level_type) {
        jl_tuplemap_level_t *cache = tc->node;
...
static void remove_methods_from_replaced_modules(jl_methtable_t *mt)
{
    ...
    jl_tuplemap_visitor(mt->defs, remove_specializations_from_replaced_modules_visitor, NULL);
    if (mt->kwsorter)
...
static jl_value_t *jl_deserialize_value_(ios_t *s, jl_value_t *vtag, jl_value_t **loc)
{
    ...
    if (vtag == (jl_value_t*)jl_simplevector_type ||
        ...
        li->tfunc = (union jl_tuplemap_t)jl_deserialize_value(s, (jl_value_t**)&li->tfunc);
        ...
        li->invokes = (union jl_tuplemap_t)jl_deserialize_value(s, (jl_value_t**)&li->invokes);
...
void jl_init_serializer(void)
{
    ...
    void *tags[] = { jl_symbol_type, jl_gensym_type, jl_datatype_type,
                     ...
                     jl_methtable_type, jl_tuplemap_level_type, jl_tuplemap_entry_type,
                     ...
                     jl_methtable_type->name, jl_tuplemap_level_type->name, jl_tuplemap_entry_type->name, jl_tvar_type->name,
