...
type InferenceState
    ...
    optimize::Bool
    inferred::Bool
    tfunc_bp::Union{TupleMapEntry, Void}
...
let stagedcache=Dict{Any,Any}()
    ...
    func_for_method(m::TupleMapEntry, tt, env) = func_for_method(m.func, tt, env)
...
function typeinf_edge(linfo::LambdaInfo, atypes::ANY, sparams::SimpleVector, needtree::Bool, optimize::Bool, cached::Bool, caller)
    ...
    if frame === nothing
        ...
        if cached
            tfunc_bp = ccall(:jl_tfunc_cache_insert, Ref{TupleMapEntry}, (Any, Any, Any, Int8), linfo.def, atypes, frame, offs)
