...
function arg_decl_parts(m::TupleMapEntry)
    tv = m.tvars
...
function kwarg_decl(m::TupleMapEntry, kwtype::DataType)
...
function show(io::IO, m::TupleMapEntry; kwtype::Nullable{DataType}=Nullable{DataType}())
...
function writemime(io::IO, ::MIME"text/html", m::TupleMapEntry; kwtype::Nullable{DataType}=Nullable{DataType}())
    tv, decls, file, line = arg_decl_parts(m)
    ft = m.sig.parameters[1]
    d1 = decls[1]
    if ft <: Function &&
...
# pretty-printing of Vector{TupleMapEntry} for output of methodswith:

function writemime(io::IO, mime::MIME"text/html", mt::AbstractVector{TupleMapEntry})
    print(io, summary(mt))
    if !isempty(mt)
        print(io, ":<ul>")
        for d in mt
...
# override usual show method for Vector{TupleMapEntry}: don't abbreviate long lists
writemime(io::IO, mime::MIME"text/plain", mt::AbstractVector{TupleMapEntry}) =
